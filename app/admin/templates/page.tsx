...
// Kelola master template catalog
// ============================================================
...
export default async function AdminTemplatesPage() {
  ...
  // Statistik pemakaian per template
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {(templates ?? []).map((t) => (
          <div
            key={t.id}
            className={[
              "bg-white border rounded-2xl p-5 transition-opacity",
            ...
            <div className="flex items-start justify-between gap-3 mb-3">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full ${tierBadge[t.tier] ?? ""}`}>
                    {t.tier}
                  </span>
                  {t.is_best_seller && (
